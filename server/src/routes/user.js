...
const FeedModel = require('../models/user');
...
class FeedDB {
    ...
    insertUser = async (item) => {
        ...
        try {
            ...
            console.log(`[Feed-DB] User Insert Error: ${e}`);
            return false;
        }
    }
    ...
    deleteUser = async (id) => {
        try {
            ...
            console.log(`[Feed-DB] User Delete Error: ${e}`);
            return false;
        }
    }
    ...
    updateUserInfo = async (item) => {
        ...
        try {
            ...
            console.log(`[Feed-DB] User InfoUpdate Error: ${e}`);
            return false;
        }
    }
    ...
    updateUserRoom = async (item) => {
        ...
        try {
            ...
            console.log(`[Feed-DB] User RoomUpdate Error: ${e}`);
            return false;
        }
    }
}
...
router.post('/addUser', async (req, res) => {
    try {
        ...
        const addResult = await feedDBInst.insertUser(req.body);
...
router.post('/deleteUser', async (req, res) => {
    try {
        const { id } = req.body;
        const deleteResult = await feedDBInst.deleteUser(id);
...
router.post('/updateUserInfo', async (req, res) => {
    try {
        ...
        const updateResult = await feedDBInst.updateUserInfo(req.body);
...
router.post('/updateUserRoom', async (req, res) => {
    try {
        ...
        const updateResult = await feedDBInst.updateUserRoom(req.body);
        if (!updateResult) return res.status(500).json({ error: dbRes.data })
        else return res.status(200).json({ isOK: true });
    } catch (e) {
        return res.status(500).json({ error: e });
